    ...
    switch (side) {
        ...
        default:
            throw std::invalid_argument(
                "Order event has an invalid side"
            );
    }
}
...
OrderType toDomainOrderType(const gridx::order::v1::OrderType orderType)
{
    ...
    switch (orderType) {
        ...
        default:
            throw std::invalid_argument(
                "Order event has an invalid order type"
            );
    }
}
...
OrderStatus toDomainOrderStatus(const gridx::order::v1::OrderStatus orderStatus)
{
    ...
    switch (orderStatus) {
        ...
        default:
            throw std::invalid_argument(
                "Order event has an invalid order status"
            );
    }
}
...
Order OrderEventMapper::toDomain(const gridx::order::v1::OrderAccepted& event) const 
{
    if (!event.has_market_id()) {
        throw std::invalid_argument(
            "Order event does not contain a market ID"
        );
    }

    if (!event.market_id().has_delivery_slot_start()) {
        throw std::invalid_argument(
            "Order event does not contain a delivery slot start"
        );
    }

    if (!event.has_expires_at()) {
        throw std::invalid_argument(
            "Order event does not contain an expiry timestamp"
        );
    }

    if (!event.has_created_at()) {
        throw std::invalid_argument(
            "Order event does not contain a creation timestamp"
        );
    }
    ...
    return Order{
        ...
        .marketId = MarketId{
            .deliverySlotStart = toTimestamp(event.market_id().delivery_slot_start())
        },
