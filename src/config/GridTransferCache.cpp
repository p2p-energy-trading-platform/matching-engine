...
std::size_t GridTransferKeyHash::operator()(const GridTransferKey& key) const noexcept {
    const auto sellerHash = std::hash<GridZoneId>{}(key.sellerGridZone);
    const auto buyerHash = std::hash<GridZoneId>{}(key.buyerGridZone);
    ...
    return sellerHash ^ (buyerHash + 0x9e3779b9U + (sellerHash << 6U) + (sellerHash >> 2U));
}
...
bool GridTransferCache::upsert(GridTransferRule rule) {
    ...
    const GridTransferKey key{.sellerGridZone = rule.sellerGridZone,
...
bool GridTransferCache::remove(const GridZoneId sellerGridZone, const GridZoneId buyerGridZone) {
    ...
    const auto removed = rules_.erase(
        GridTransferKey{.sellerGridZone = sellerGridZone, .buyerGridZone = buyerGridZone});
...
std::optional<GridTransferRule> GridTransferCache::find(const GridZoneId sellerGridZone,
                                                        const GridZoneId buyerGridZone) const {
    if (sellerGridZone == buyerGridZone) {
        return GridTransferRule{.sellerGridZone = sellerGridZone,
    ...
    const auto iterator = rules_.find(
        GridTransferKey{.sellerGridZone = sellerGridZone, .buyerGridZone = buyerGridZone});
...
GridTransferRule GridTransferCache::resolve(const GridZoneId sellerGridZone,
                                            const GridZoneId buyerGridZone) const {
    if (sellerGridZone == buyerGridZone) {
        return GridTransferRule{.sellerGridZone = sellerGridZone,
                                .buyerGridZone = buyerGridZone,
